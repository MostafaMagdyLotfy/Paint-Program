 ...
 */
public class Elipse implements Shape,Cloneable {
 private Point p;
private Color c;
private Color fc;
...
private double radius1;
   private double radius2;
    ...
    public void draw(Graphics canvas) {
      ...
      g.setStroke(bs);
      g.setColor(getColor());
    radius1=m.get("radius1");
            radius2=m.get("radius2");
        int Radius1= (int)radius2;
        int Radius2=(int)radius1;
          g.setColor(getColor());
          if(this.fc==null)
   g.draw(new Ellipse2D.Double(p.x, p.y,Radius2,Radius1));
              else
              g.fill(new Ellipse2D.Double(p.x, p.y,Radius2,Radius1));
...
//To change body of generated methods, choose Tools | Templates.
    ...
    {
        Elipse circle = (Elipse) super.clone();
    ...
    @Override
     public boolean contains(Point zx){
        return (p.getX() < zx.getX() &&p.getY() < zx.getY() &&
                p.getX() + radius1 > zx.getX()  &&
           p.getY() + radius2 > zx.getY());
    }
